...
async def extract_emails(string):
    pattern = r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b'
    emails = re.findall(pattern, string)
    return emails

async def convert_to_email_format(email):
    suffixes = [
...
async def clear_emails(emails):
    cleared_emails = []

    for email in emails:
        formatted_email = await convert_to_email_format(email)
        if formatted_email:
